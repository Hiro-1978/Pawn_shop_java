...
public class pawnhis extends javax.swing.JInternalFrame {
    ...
    private void initComponents() {
        ...
        jLabel2.setText("ประวัติการจำนำ");
        jPanel1.add(jLabel2, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 10, 310, 50));
        ...
        jPanel1.add(datebegin, new org.netbeans.lib.awtextra.AbsoluteConstraints(470, 80, 150, 30));
        ...
        jPanel1.add(dateend, new org.netbeans.lib.awtextra.AbsoluteConstraints(690, 80, 150, 30));
        ...
        jPanel1.add(btnok, new org.netbeans.lib.awtextra.AbsoluteConstraints(850, 80, -1, -1));
    ...
    private void txtsearchKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtsearchKeyReleased
        try {
            String sql = "SELECT `p_no`AS เลขที่ตั๋วจำนำ, `m_id`AS รหัสลูกค้า, `m_name`AS ชื่อลูกค้า, `t_name`AS ชื่อสินค้า, `t_type`AS รหัสสินค้า, `t_date`AS วันที่จำนำ, `t_end`AS วันหมดอายุ"
                    + ", `t_price`AS ราคา FROM `pawn` WHERE m_id LIKE '%" + txtsearch.getText() + "%'";
    ...
    private void lendFilter() {
        ...
        if (FDate.length() > 0 && TDate.isEmpty() && FDate.isEmpty() && TDate.length() > 0) {
            ...
            try {
                String reportSql = "SELECT `p_no`AS เลขที่ตั๋วจำนำ, `m_id`AS รหัสลูกค้า, `m_name`AS ชื่อลูกค้า, `t_name`AS ชื่อสินค้า, `t_type`AS รหัสสินค้า, `t_date`AS วันที่จำนำ, `t_end`AS วันหมดอายุ"
                    + ", `t_price`AS ราคา FROM `pawn` WHERE m_id LIKE '%" + txtsearch.getText() + "%' and t_date between'" + FDate + "' and '" + TDate + "' ";
